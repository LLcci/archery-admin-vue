;(() => {
  ...
  window.CAP_CUSTOM_WASM_URL ||
    [`/archery-admin-vue/cap_wasm.min.js`, `/archery-admin-vue/cap_wasm_bg.wasm`].forEach((e) => {
    ...
    attributeChangedCallback(e, t, r) {
      ...
      ;('data-cap-worker-count' === e && this.setWorkersCount(parseInt(r, 10)),
        'data-cap-i18n-initial-state' === e &&
          ...
          (this.#i.querySelector('p').innerText = this.getI18nText('initial-state', "I'm a human")))
    }
      ...
      if (!this.#n)
        try {
          ...
          try {
            ...
            const { challenge: s, token: i } = await (
    ...
    async solveChallenges(t) {
      ...
      const i = Array(this.#r)
        ...
        a = ([t, a], n) =>
          new Promise((o, d) => {
            const c = i[n]
            if (!c) return void d(new Error('Worker not available'))
            const l = setTimeout(() => {
              try {
                ;(c.terminate(), (i[n] = new Worker(this.#e)))
              } catch (e) {
            ...
            if (
              ...
              (c.onerror = (e) => {
                ;(clearTimeout(l), this.error(`Error in worker: ${e.message || e}`), d(e))
              }),
              c.postMessage({
                salt: t,
                target: a,
                wasmUrl:
                  window.CAP_CUSTOM_WASM_URL ||
                  `https://cdn.jsdelivr.net/npm/@cap.js/wasm@${e}/browser/cap_wasm.min.js`
              }),
